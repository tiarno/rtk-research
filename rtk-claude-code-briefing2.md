...
## Delivery path: how the attack data actually reaches the device

This came up as a direct challenge to the whole line of research — is this academic-only, or can the malicious bytes actually reach a real device? Researched and resolved with real evidence; findings below are load-bearing for the talk's credibility.

**Correction to an earlier assumption:** NTRIP is not universally unauthenticated. Real casters split into **open** (free/public, genuinely no auth) and **closed** (commercial, HTTP Basic Auth, increasingly offer TLS on 443 alongside plaintext 2101).

**The actual load-bearing finding:** RTKLIB-class devices routinely skip TLS entirely, regardless of what the caster offers. Confirmed via **RTK2GO** (a real, currently-operating public NTRIP caster) documentation stating that secure NTRIP Rev2/TLS adoption is under 1-in-10 devices, and explicitly naming lower-end devices — including ones using u-blox F9 modules or open-source code such as RTKLIB — as the class that often doesn't support secure connections at all. This is a citable, attributable claim from a named operator of live infrastructure, not an inference from an old GitHub issue (though issue #450 on tomojitakasu/RTKLIB from 2019 shows the same problem reported years earlier). Practical implication: even authenticated/TLS-capable casters don't protect an RTKLIB-based client that can only speak plaintext — credentials and the RTCM3 stream itself are both sniffable/modifiable on-path.

**Two real, mainstream (not fringe) delivery paths:**
- **UHF/VHF radio link** (410–470MHz typical, some 450/900MHz; range 1–5km, up to ~10km line-of-sight) — base-to-rover corrections with zero internet/caster involvement, standard/actively-sold option specifically for remote or no-signal sites (survey, construction). Most exposed path: no infrastructure to compromise, just RF in range on a known frequency.
- **NTRIP over cellular/WiFi/Ethernet** — either on-path MITM of the rover's connection to the real caster (rogue WiFi AP / ARP spoofing; realistic against plaintext given the TLS-adoption gap above), or redirect the rover to an attacker-controlled caster entirely via DNS/config tampering (no interception needed — attacker just becomes the destination).

**Full mechanical chain, delivery → overflow** (walks through how Path A/B above connects to the `decode_type1033` bug in Task 1):
1. Attacker gets positioned (radio range, or on-path/impersonating on the network) — this is the only step that differs between UHF and NTRIP paths.
2. Bytes reach RTKLIB's stream-decoding layer — same endpoint regardless of transport; RTKLIB doesn't distinguish by transport at this layer.
3. Outer RTCM3 framing check (preamble 0xD3, length, message-type ID, payload, CRC24Q checksum) — this is a data-integrity check, not authentication. CRC24Q is a public, computable algorithm; attacker can produce a valid checksum for any payload.
4. RTKLIB dispatches purely on the message-type field — setting type=1033 routes to `decode_type1033` regardless of whether the payload represents anything physically real.
5. The overflow: attacker-controlled 8-bit length byte `strncpy`'d into a fixed 64-byte buffer, no bounds check → ~191-byte overflow at length=255.

**Key point for the talk:** the delivery path only decides how bytes reach the device — steps 2–5 are identical regardless of transport. The vulnerability is transport-agnostic: no authentication exists anywhere in the chain, on any realistic delivery method.

**Net effect on the "is this real" question:** the delivery path is now the *best*-evidenced part of the whole research (two independently solid paths, one backed by a named live-infrastructure operator's own words). The parts that remain genuinely open are unaffected by this: exploitability of `decode_type1033` past a crash (Task 1), and real-system fail-open/fail-closed behavior (Task 2).
